...
IPCA <- read_excel("C:/Users/Henrique/Desktop/Econometria/11-Sazonalidade-Suavizacao/IPCA.xls")
View(IPCA)
                                      #Cria um gráfico do arquivo
MM <- data.frame(na.omit(ma(IPCA$IPCA,order = 12, centre = T)))   #Cria  uma serie de médias móveis tradicionais omitindo oa NAS de lag(N)=12    

a <- (127-nrow(MM))+1                                                #Define um parâmetro a para ponderar as perdas de dados para ponderação da média
IPCA.DF <- as.data.frame(IPCA$IPCA[a:127])                        #Define IPCA.DF como uma vetor do mesmo tamanho que o vetor das médias móveis MM
...
Inflacao <- ts(IPCA$Ano.M�s, start = 2008, frequency = 12)
...
Tab_Dados2 <- data.frame(IPCA$Ano.M�s, Tendencia)
